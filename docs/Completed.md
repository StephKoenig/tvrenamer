...
### 55) Subtitle merge v2 — source-side everything, unified progress, per-row counter
- **Why:** v1 of subtitle merging shipped in v1.0.274 was useful but had three real-world rough edges: tool detection failed on MP4Box (wrong probe flag), idempotency parsers were written against imagined tool output rather than real captures, and the merge ran on the destination disk (slow over NAS/SMB targets, no per-row progress visible to the user). The user-driven testing in this cycle exposed each of those, and the architecture was redesigned around three principles — local-first I/O, unified accounting, and visible progress.
- **Where:** `MoveRunner.runSourceSideMerge` (new pre-batch task), `FileMover.doActualMove` (relocated tag step + `consumedByMerge` short-circuit), new `WorkPlan` and `RowPhase` model classes, new `SubtitleMergeProgressListener` interface with per-file progress, `Mp4SubtitleMerger` and `MkvSubtitleMerger` updated for streaming progress parse, `ProcessRunner.runStreaming` new variant, `ResultsTable` driving the bottom progress bar from the WorkPlan and rendering per-row percentage labels.
- **What we did:**
  - **Tool-detection bug fix:** `ExternalToolDetector.isExecutableInPath` now tries `--version` first and falls back to `-version` (single-dash, GPAC convention) so MP4Box is detected on Windows.
  - **Idempotency parser fixes:** rewrote both detectors using realistic tool output captured from a real run. MKV scans the JSON `tracks[]` by splitting on `"id":` and matching `"type":"subtitles"` together with `"language":"<code>"` per track block; MP4 splits MP4Box `-info` output on `# Track ` boundaries and accepts both modern (`Media Type: text:...` + separate `Media Language` line) and legacy (`Subtitle in language: <code>`) phrasing.
  - **Source-side merge:** a synthetic `SourceSideMergeTask` is submitted as the first callable on the single-thread executor, before any per-file `FileMover`. FIFO ordering guarantees the merge phase finishes before any move starts. Pairs are matched by `(destDir, canonicalBaseName)` using `FileMover.getDesiredDestName` so subtitles with scrambled source filenames still pair with their canonical media destination. On `SUCCESS` (or "language already present"), the destination path is added to `sourceMergedDestinations` so the post-batch step skips it without redundant network identification calls. With `deleteSubtitlesAfterMerge` on, the source `.srt` is deleted and its FileMover is marked `consumedByMerge=true` — that FileMover's `call()` short-circuits to a successful no-op.
  - **Local tagging:** metadata tag now runs in `doActualMove` BEFORE the rename/copy, on the source path. The metadata tool reads/writes locally instead of over the network. Tag tick is gated on `TaggingResult.SUCCESS` or `.FAILED` so non-taggable extensions (e.g. `.srt`) don't tick a phantom unit that wasn't predicted in the work plan.
  - **Unified progress bar via WorkPlan:** new `WorkPlan` model class holds total + completed unit counters, with a thread-safe progress listener. `ResultsTable` predicts total ops at start (1 move per file + 1 tag per taggable file + 1 merge per actually-paired media) and creates a `WorkPlan`. The bottom progress bar's max + selection is driven entirely by the plan's progress callback. `FileMover` ticks after move + tag, `MoveRunner` ticks per merge in both source-side and post-batch. `WorkPlan.completeAll` at end of run is a safety net against any over/under-prediction mismatch. Byte-based copy progress is suppressed from the main bar while a WorkPlan is active (per-row copy percentages still display).
  - **Per-row state taxonomy:** new `ItemState` entries `MOVING` (arrow-right), `TAGGING` (tag-pencil), `MERGING` (video-rect) using existing SweetieLegacy icons. New `RowPhase` enum + `MoveObserver.onPhaseChange` lets `FileMover` signal sub-phase transitions to the UI without leaking view-package types into the controller. `FileMonitor` translates phases to icons on the SWT thread.
  - **Per-row merge percentage overlay:** `SubtitleMerger.merge` gains an optional `IntConsumer onProgress`; mergers parse mkvmerge `--gui-mode` `#GUI#progress NN%` lines and MP4Box `ISO File Writing: |...| (NN/100)` lines via `ProcessRunner.runStreaming`. `ResultsTable.TableSubtitleMergeListener` creates a percentage Label overlay on the row's status cell at start, updates it on each progress tick, and disposes it at finish — same machinery as `FileMonitor`'s existing copy-progress overlay.
  - **Post-batch becomes a fallback** for the cases source-side can't satisfy (subtitle in a different source dir from media). It now pre-filters the candidate set by container extension so non-media files don't trigger spurious listener fires or work-plan ticks.
- **Notes:**
  - Net behaviour on a typical batch (3 paired media + 3 subtitles, tag + merge + delete-after-merge enabled, cross-volume move): every rewrite-heavy I/O happens locally on the source disk, the move itself is the only network operation. Bar advances 0% → 100% across 12 ticks (6 moves + 3 tags + 3 merges).
...
### 57) Auto re-match affected/unmatched rows when Matching preferences are saved
- **Why:** Changing a show-name override or a disambiguation on the Preferences → *Matching* tab only re-drove lookups for rows that had *previously failed* to find a show. A row already matched to a series was never re-pointed even when a new override should send it elsewhere, so the user had to remove and re-add files. (The *Renaming* tab already auto-refreshes proposed names in place via `refreshDestinations()`, so no change was needed there.)
- **Where:** `FileEpisode.rematchWouldChangeResult(UserPreferences)` (new pure predicate); `ResultsTable.rematchRows(Predicate<FileEpisode>)` (new shared engine), the `SHOW_NAME_OVERRIDES` case in `ResultsTable.updateUserPreferences`, and `showBatchDisambiguationDialogIfNeeded`. Tests in `FileEpisodeRematchTest`.
- **What we did:**
  - Added a pure, unit-tested predicate on `FileEpisode` that selects a row for re-match if it is **unmatched**, if a **name override** now resolves its extracted name to a different effective name, or if a **disambiguation override** now pins a provider series id different from the one currently matched. It compares resolution *inputs* (resolved name + pinned id) against current state, so rows whose outcome cannot change are skipped with no provider query.
  - Extracted the per-row re-match logic (previously duplicated across the unfound-only retry and the post-disambiguation batch loop) into one `rematchRows(selector)` engine. Rows are re-resolved by their effective (override-applied) name so name overrides and disambiguation overrides key consistently with the parse-time lookup — fixing a latent keying mismatch where the batch loop mapped by the raw extracted name.
  - Repointed the `SHOW_NAME_OVERRIDES` event to `rematchRows(ep -> ep.rematchWouldChangeResult(prefs))`; retired `retryUnfoundShowsAfterOverrideChange()` (its unmatched case is now clause 1 of the predicate).
  - Added a `suppressOverrideRematch` guard: the batch dialog persists overrides per selection (each firing `SHOW_NAME_OVERRIDES`), so the event-driven pass is suppressed during that window and the dialog runs a single explicit affected-row re-match afterwards — avoiding N redundant passes and re-entrant dialog risk.
- **Notes:**
  - Listings are cached per `Series`, so re-matching an already-resolved show does not re-fetch listings; only a provider *search* re-fires, and only for rows the predicate selects. Persisted disambiguation choices resolve silently (no re-prompt).
  - Re-matching a row runs `FileEpisode.listingsComplete()`, which resets `chosenEpisode` to 0 — a user's manual pick in a multi-option row's Combo is discarded, but only for rows actually re-matched. Accepted tradeoff.
  - `savePreferences` calls both override setters unconditionally, so a Matching-tab save fires the event twice; each pass is bounded to affected rows and `ShowName` coalesces concurrent lookups for the same query string, so the second pass is effectively a no-op at the network layer.
